...
impl Protocol for TelnetProtocol {
    ...
    fn description(&self) -> &'static str { "Telnet login prompt" }
    ...
    async fn authenticate(
        ...
        let addr = addr_str.to_socket_addrs().map_err(|e| ZeusError::Network(e))?
        ...
        // Read login prompt
        let prompt = conn.read_until_crlf().await.map_err(|e| ZeusError::Protocol(e.to_string()))?;
        ...
        // Read response
        let resp = conn.read_until_crlf().await.map_err(|e| ZeusError::Protocol(e.to_string()))?;
