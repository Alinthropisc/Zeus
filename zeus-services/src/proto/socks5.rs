...
impl Protocol for Socks5Protocol {
    ...
    fn description(&self) -> &'static str {
        "SOCKS5 username/password authentication (RFC 1928/1929)"
    }
    ...
    async fn authenticate(
        ...
        if cred.username.len() > 255 || cred.password.len() > 255 {
            return Ok(AttackResult::Error(
                "Credential exceeds SOCKS5 255-byte limit".into(),
            ));
        }
        ...
        conn.write_all(&[0x05, 0x02, 0x00, 0x02])
            .await
            .map_err(|e| ZeusError::Protocol(e.to_string()))?;

        let server_choice = conn
            .read_until_crlf()
            .await
            .map_err(|e| ZeusError::Protocol(e.to_string()))?;
        ...
        if server_choice[1] == 0xFF {
            let _ = conn.shutdown().await;
            return Ok(AttackResult::Error(
                "SOCKS5 rejected all auth methods".into(),
            ));
        }
        ...
        if server_choice[1] == 0x00 {
            let _ = conn.shutdown().await;
            return Ok(AttackResult::Error(
                "SOCKS5 requires no authentication (open proxy)".into(),
            ));
        }
