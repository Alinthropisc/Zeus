...
#include "../../zeus-core/lib/zeus_crypto.hh"
...
//    };
    ...
    class ZeusDatabaseService : public zeus::proto::ZeusProtocolService
    {
        ...
        protected:
            ...
            [[nodiscard]]
            virtual std::unique_ptr<zeus::crypto::ZeusSaslExchange> select_mechanism(std::string_view server_hello, std::string_view user,std::string_view password) const = 0;
            ...
            [[nodiscard]]
            zeus::ZeusAuthResult perform_sasl_exchange(zeus::proto::ZeusPacketCodec& codec, ZeusConnection& conn,zeus::crypto::ZeusSaslExchange& exchange)
            {
                ...
                if (!conn.send(raw))
                {
                    return zeus::ZeusAuthResult::protocol_error;
                }
                ...
                if (!n)
                {
                    return zeus::ZeusAuthResult::retry; // timeout: server busy, requeue
                }
                ...
                if (!n)
                {
                    return zeus::ZeusAuthResult::retry;
                }
                ...
                return ok ? zeus::ZeusAuthResult::success : zeus::ZeusAuthResult::failure;
            }
